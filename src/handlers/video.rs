...
pub fn router() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route("/video/{id}/download", get(download_video))
}
...
async fn list_videos(
    ...
    let items: Result<Vec<VideoItem>, _> = if auth_user.can_view_all_media() {
        sqlx::query_as(
            "SELECT id, user_id, title, original_filename, stored_path, size_bytes, mime_type FROM videos",
        )
        .fetch_all(&state.db.pool)
        .await
    } else {
        sqlx::query_as(
            "SELECT id, user_id, title, original_filename, stored_path, size_bytes, mime_type FROM videos WHERE user_id = ?",
        )
        .bind(auth_user.id)
        .fetch_all(&state.db.pool)
        .await
...
// POST /api/video - Upload video (multipart/form-data, no size limit)
async fn upload_video(
    ...
    // No size limit for video
    let size_bytes = file_bytes.len() as u64;

    // Generate storage path
    let (stored_path, full_path) =
        generate_storage_path(&state.config.media_dir, MediaType::Video, &extension);
    ...
    let result = sqlx::query(
        "INSERT INTO videos (user_id, title, original_filename, stored_path, size_bytes, mime_type) VALUES (?, ?, ?, ?, ?, ?)",
    )
    .bind(auth_user.id)
    .bind(&title)
    .bind(&orig_filename)
...
async fn get_video(
    ...
    let item: Result<VideoItem, _> = sqlx::query_as(
        "SELECT id, user_id, title, original_filename, stored_path, size_bytes, mime_type FROM videos WHERE id = ?",
    )
    .bind(id)
...
// GET /api/video/:id/download - Download the actual video file
